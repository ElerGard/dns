...
def inser_csv_to_db(files: list, conn, conn_sqlalchemy) -> None:
    ...
    # На моей системе при довольно большой нагрузке файл t_sales добавляется за 5 минут в среднем
    # Считываются и записываются файлы
...
def first_part(conn_sqlalchemy) -> None:
    ...
    task1 = ( 
        ...
        # поэтому прибыли с него не было. Таже не было условий на счёт товаров с названиями например "Доставка ..."
        # они также учитываются, но это исправляется дополнением в условие where
        ...
        '''SELECT branches.Индекс AS ФилиалИД, products.Индекс AS ТоварИД, branches.Наименование, 
            ...
            LIMIT 10
        ''',
        # Если делать следующий запрос напрямую в базе, на моей системе скорость выше в 5 раз.
        # Такая разница только на нём, остальные +- по времени похожи
    ...
    # Немного не понял что именно требуется, поэтому сделал два разных графика основанных на втором задании и по всем данным
